...
function restrictPositionCoord(
	...
	const scaledImage = imageSize * zoom;

	// if the image is smaller than the crop area, we can't move the image
	if (scaledImage <= cropSize) {
		return (cropSize - imageSize) / 2;
	}

	// Should not be able to pan in in the positive direction further than the image edge
	const maxPosition = (scaledImage - cropSize) / 2;

	// Should not be able to pan in in the negative direction further than the image edge
