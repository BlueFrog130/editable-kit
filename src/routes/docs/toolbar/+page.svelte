<script lang="ts">
	...
	let toolbarState: EditableState | undefined = $state();
...
<section class="mb-16">
	...
	>
		<Editable.Root {editing}>
			{#snippet children({ state, save: saveFn })}
				{((save = saveFn), (toolbarState = state), '')}
				{#if editing && state}
				...
				>
					{#snippet children({ rich })}
						<div class="text-muted-foreground">{@render rich('body')}</div>
					{/snippet}
				</Editable.Data>
			{/snippet}
		</Editable.Root>
...
<section class="mb-16">
	...
	<div class="mt-6 space-y-3">
		...
		<div class="rounded-lg border border-border p-4">
			<p class="font-mono text-sm">state.text(fn)</p>
			<p class="mt-1 text-sm text-muted-foreground">
				...
				>
				instance. Returns <code class="font-mono">undefined</code> if no text editor is active.
			</p>
		</div>
		<div class="rounded-lg border border-border p-4">
