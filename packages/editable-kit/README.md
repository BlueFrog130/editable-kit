...
| `Editable.Image`     | one `image` node                   | Image with pan/zoom cropping and WebP export |
...
emits them, so images reserve their space before loading.
...
```ts
import { nodeDefaults } from 'editable-kit';
...
import * as Editable from 'editable-kit';
import { Root, Text, Multiline, Rich, Image, Field, EditableState, Renderer } from 'editable-kit';
import { text, paragraphs, image, imageAttrs } from 'editable-kit'; // document helpers
