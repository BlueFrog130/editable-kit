<script lang="ts">
	...
	import { pickFile } from '$lib/components/field/pick-file.js';
	...
	const imageActive = $derived(toolbarState.isActive('image'));
	$inspect({ imageActive });
...
<div class="fixed top-16 right-0 left-0 z-40 mx-auto max-w-xl px-4">
	...
	>
			...
			{#snippet menu(action, { close })}
				<form
					...
					onsubmit={(e) => {
						...
						const closeFn = close(() => {
							const chain = toolbarState.run((e) => e.chain().focus().extendMarkRange('link'));
				...
				>
					<input
						...
						name="href"
						aria-label="Link URL"
						value={toolbarState.run((e) => e.getAttributes('link').href)}
					/>
					<button
						type="submit"
						class="rounded p-2 disabled:text-neutral-400"
					...
					<button
						...
						onclick={close(() => toolbarState.run((e) => e?.chain().focus().unsetLink().run()))}
					>
		...
		<button
			class="rounded p-2 disabled:text-neutral-400"
			disabled={!toolbarState.isActive('image')}
			onclick={async () => {
				const file = await pickFile();
				if (!file) return;
				const src = await uploadAsDataURL(file);
				toolbarState.run((e) => e.commands.setImage({ src }));
			}}
			...
			{#snippet menu(action, { close })}
				<form
					...
					onsubmit={(e) => {
						...
						const closeFn = close(() => {
							toolbarState.run((e) => e.commands.updateAttributes('image', { alt }));
						});
						closeFn();
					}}
				>
					<input
						...
						name="alt"
						aria-label="Image alt text"
						value={toolbarState.run((e) => e.getAttributes('image').alt)}
					/>
					<button
						type="submit"
						class="rounded p-2 disabled:text-neutral-400"
