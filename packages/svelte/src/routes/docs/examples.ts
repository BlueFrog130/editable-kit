...
export const TYPE_SAFETY = `import { text, paragraphs, image } from '@editable-kit/svelte';
import type { ProseMirrorJSON } from '@editable-kit/svelte';
...
export const RENDERER_BASIC = `<script lang="ts">
  import { Renderer } from '@editable-kit/svelte';
</script>
...
// Theming examples
export const THEMING_CSS_VARS = `/* Override CSS custom properties on any parent element */
.my-editor {
  --ek-placeholder-color: #94a3b8;              /* Empty editor placeholder text */
  --ek-placeholder-font-style: italic;          /* Placeholder font style */
  --ek-image-placeholder-height: 12rem;         /* Empty image field height */
  --ek-image-placeholder-background: #f1f5f9;   /* Empty image field fill */
  --ek-image-placeholder-radius: 0.5rem;        /* Empty image field corners */
}`;

export const THEMING_SCOPED = `<!-- Scope theme to a specific section -->
<div style="--ek-placeholder-color: #94a3b8; --ek-image-placeholder-height: 12rem;">
...
export const THEMING_DARK_MODE = `/* Tailwind v4 dark mode with CSS custom properties */
:root {
  --ek-placeholder-color: #9ca3af;
  --ek-placeholder-font-style: normal;
  --ek-image-placeholder-background: #f1f3f5;
}

@media (prefers-color-scheme: dark) {
  :root {
    --ek-placeholder-color: #6b7280;
    --ek-image-placeholder-background: #1f2937;
  }
}`;

export const THEMING_ALL_VARIABLES = `/* Every variable editable-kit reads, with its default */
--ek-placeholder-color: #adb5bd;
--ek-placeholder-font-style: normal;
--ek-image-placeholder-height: 8rem;
--ek-image-placeholder-background: #f1f3f5;
--ek-image-placeholder-radius: 0.25rem;`;

export const THEMING_IMAGE_CUSTOM = `/* The field element is the same node in both modes — style it once.
   Sizing lives here, not in the library: the field has no wrapper. */
[data-ek-variant='image'] {
  aspect-ratio: 16 / 9;
  overflow: hidden;
  border-radius: 0.5rem;
}

[data-ek-variant='image'] img {
  width: 100%;
  height: 100%;
  object-fit: cover;
}

/* Shown only while the field has no image */
[data-ek-variant='image']:not(:has(img)) {
  --ek-image-placeholder-height: 12rem;
  --ek-image-placeholder-background: #f1f5f9;
  --ek-image-placeholder-radius: 0.5rem;
}`;
