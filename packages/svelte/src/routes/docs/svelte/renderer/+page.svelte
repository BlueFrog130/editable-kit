...
<section class="mb-16">
	...
	{#snippet heading(node: import('$lib/index.js').HeadingNode, children: import('svelte').Snippet)}
		<h2
	...
	{#snippet link(mark: import('$lib/index.js').LinkMark, children: import('svelte').Snippet)}
		<a
...
<!-- Overrides on Root -->
<section class="mb-16">
	<h2 class="font-serif text-2xl tracking-tight">Overrides on Root</h2>
	<p class="mt-3 mb-4 text-sm text-muted-foreground">
		<code class="rounded bg-muted px-1.5 py-0.5 font-mono text-sm">Root</code> and every field
		accept the same
		<code class="rounded bg-muted px-1.5 py-0.5 font-mono text-sm">overrides</code> prop; set it once
		on Root and the fields inherit it. Fields render through the Renderer when not editing, so your overrides
		apply to the read-only display and to the loading state while TipTap initializes.
	</p>

	{#snippet editableHeading(
		node: import('$lib/index.js').HeadingNode,
	...
	{#snippet editableLink(
		mark: import('$lib/index.js').LinkMark,
		children: import('svelte').Snippet
	)}
		<a
			href={mark.attrs.href}
	...
	{#snippet editableBold(
		_mark: import('$lib/index.js').BoldMark,
		children: import('svelte').Snippet
	)}
	...
	>
		{#snippet children({ save })}
			...
			>
				<div class="space-y-4">
					<Editable.Text bind:value={editableData.title} aria-label="Post title" />
					<Editable.Rich bind:value={editableData.body} aria-label="Post body" />
				</div>
			</Example>
		{/snippet}
	</Editable.Root>
		...
		<ul class="mt-3 space-y-1.5 text-sm text-muted-foreground">
			...
			<li>
				Define overrides once and pass them to <code
					class="rounded bg-muted px-1.5 py-0.5 font-mono text-sm">Editable.Root</code
